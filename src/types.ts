...
export type ProfileDto = Pick<Profile, 'user_id' | 'email' | 'display_name' | 'locale' | 'timezone' | 'created_at' | 'updated_at' | 'deleted_at' | 'scheduled_for_deletion_until'>;

// Habit DTO: Includes core habit fields, excludes deleted_at and trash_expires_at for active views
export type HabitDto = Pick<Habit, 'habit_id' | 'name' | 'description' | 'frequency_type' | 'target_value' | 'unit_kind' | 'value_min' | 'value_max' | 'times_per_week' | 'week_days' | 'tags' | 'created_at' | 'updated_at'> & {
  params?: HabitParam[]; // Associated params
};

// Habit List Response DTO
export type HabitListDto = {
  habits: HabitDto[];
  total: number;
  page: number;
  limit: number;
...
export type HabitParamDto = Pick<HabitParam, 'param_id' | 'habit_id' | 'effective_from' | 'effective_until' | 'target_value' | 'value_min' | 'value_max' | 'created_at'>;

// Checkin Portion DTO: Excludes server/internal fields for client
export type CheckinPortionDto = Pick<CheckinPortion, 'portion_id' | 'habit_id' | 'local_date' | 'value' | 'note' | 'client_command_uuid' | 'created_at' | 'updated_at' | 'deleted_at' | 'trash_expires_at'>;
...
export type DsarDto = Pick<DsarRequest, 'request_id' | 'request_type' | 'status' | 'priority' | 'request_details' | 'resume_checkpoint' | 'throttle_mode' | 'created_at' | 'updated_at' | 'closed_at' | 'delivered_at'>;

// Consent DTO
export type ConsentDto = Pick<Consent, 'consent_id' | 'consent_type' | 'consent_text' | 'consent_version' | 'granted' | 'granted_at' | 'revoked_at' | 'created_at'>;

// Notification DTO
export type NotificationDto = Pick<Notification, 'notification_id' | 'type' | 'enabled' | 'digest_hour' | 'quiet_hours_start' | 'quiet_hours_end' | 'silent_days' | 'created_at' | 'updated_at'>;

// Audit Log DTO: Masked PII, excludes sensitive fields like ip_address, user_agent if needed
export type AuditLogDto = Pick<AuditLog, 'log_id' | 'action' | 'resource_type' | 'resource_id' | 'old_values' | 'new_values' | 'created_at'> & {
  user_id?: string; // Optional for self-view
};

// Sync Log DTO
export type SyncLogDto = Pick<SyncLog, 'sync_id' | 'command_uuid' | 'command_type' | 'device_id' | 'status' | 'error_message' | 'created_at' | 'applied_at'>;
...
export type CreateHabitCommand = Pick<Habit, 'name' | 'description' | 'frequency_type' | 'target_value' | 'unit_kind' | 'value_min' | 'value_max' | 'times_per_week' | 'week_days'> & {
...
export type CreateHabitParamCommand = Pick<HabitParam, 'effective_from' | 'effective_until' | 'target_value' | 'value_min' | 'value_max'> & {
  habit_id: string;
};
...
export type CreatePatTokenCommand = {
  name: string;
  scopes: string[];
  ttl_hours?: number;
};
...
export type CreateIcsTokenCommand = {
  name: string;
};

// Create DSAR Command
export type CreateDsarCommand = {
...
export type UpdateNotificationCommand = Partial<Pick<Notification, 'enabled' | 'digest_hour' | 'quiet_hours_start' | 'quiet_hours_end' | 'silent_days'>> & {
  notification_id: string;
};

// Create Export Command
export type CreateExportCommand = {
  start_date: string;
  end_date: string;
  format: 'csv' | 'json';
  raw: boolean;
